...
public class TheBuyingAgent {
	...
	public void InitialiseAndLaunch(){
		 ...
		 try {
			...
			// the average price calculation method should be used within this class
			 
				
			// this should loop independantly from any other statement	
			HandleMessages.MonitorReadRespondModify(f, "BuyingAgent"); // deals with any incoming messages and interprets them
			PerfromRecomendation();	// performs recommendations and generates XML files as a message
	...
	//this is used as part of implmenting the decision making ability, done by comparing information within the database and producing recommendations accordingly
	private void PerfromRecomendation() throws ParserConfigurationException, IOException{
		...
		try{
		String stmnt = "SELECT ProductName FROM ToBeBoughtProducts  WHERE ProductStatus = 'True'"; // identifies which product to deal with
		ResultSet rs= Execute.ExecuteDBQuery(stmnt); // execute the statement
		...
		while(rs.next())  {
			...
			while(rs3.next()){
				...
				//The recommendation criterias used below are only tempoorarly and can be expanded upon later, however they are sufficent to demonstrate the behaviour of the system
